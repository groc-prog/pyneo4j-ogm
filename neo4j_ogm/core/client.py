...
class Neo4jClient:
        ...
        for model in models:
            if issubclass(model, (NodeModel, RelationshipModel)):
                ...
                for property_name, property_definition in model.__fields__.items():
                    ...
                    labels_or_type = (
                        list(getattr(model.model_settings, "labels"))
                        if issubclass(model, NodeModel)
                        else getattr(model.model_settings, "type")
                    )
    ...
    def _resolve_database_model(
        ...
        for model in list(self.models):
            ...
            if issubclass(model, NodeModel):
                model_labels = set(getattr(model.model_settings, "labels"))
            elif issubclass(model, RelationshipModel):
                model_labels = set(getattr(model.model_settings, "type"))
